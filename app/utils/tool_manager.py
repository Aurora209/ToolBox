...
import os
import subprocess
from tkinter import messagebox, filedialog, Toplevel, Label, Entry, Button, StringVar
...
def run_tool(app, path):
    ...
    if os.path.exists(path):
        try:
            ...
            for tool in app.current_displayed_tools:
                if tool['path'] == path:
                    from ..services.tool_scanner import record_tool_usage
                    record_tool_usage(app, tool['path'], tool['name'] + tool['ext'], tool['category'])
                    break
...
def rename_tool(app, path, current_name):
    ...
    if new_name and new_name != current_name:
        ...
        if path in app.tools_added_record:
            app.tools_added_record[path]['name'] = new_name
...
def edit_note(app, path):
    ...
    if note is not None:
        ...
        app.config_manager.save_config()
        if path in app.tools_added_record:
            parts = app.config['ToolAddedRecord'][path].split('|', 5)
            parts[4] = note.strip()
